...
class SerializerMacro {
	...
	static public function _build(fields:Array<Field>, ?customTypes:Array<NetworkType>, ?className:String):Array<Field> {
		...
		networkTypes.push({
			name: "String",
			serialize: function(varNameOut:String) {
				return [macro bo.writeInt16($i{varNameOut}.length), macro bo.writeString($i{varNameOut})];
			},
			unserialize: function(varNameIn:String) {
		...
		networkTypes.push({
			...
			serialize: function(varNameOut:String) {
				return [
					macro bo.writeInt32($i{varNameOut}.length),
		...
		networkTypes.push({
			...
			serialize: function(varNameOut:String) {
				return [
					macro bo.writeInt32($i{varNameOut}.length),
					macro for (i in 0...$i{varNameOut}.length) {
						bo.writeInt16($i{varNameOut}[i].length);
						bo.writeString($i{varNameOut}[i]);
					}
				];
			},
			unserialize: function(varNameIn:String) {
				return [
					macro $i{varNameIn} = [for (i in 0...bi.readInt32()) bi.readString(bi.readInt16())]
				];
			}
		});
		...
		if (networkVariables.length == 0) {
			#if debug
			trace('No serialization for $className');
			#end
		}
		...
		var sid = -1;
		if (networkVariables.length > 0)
			sid = getClassSerializedId();
		var def = macro class {
			public var _sid:Int = $v{sid};
		...
		for (netVar in networkVariables) {
			...
			if (netVar.redirection != null)
				varNameIn = netVar.redirection;
		...
		var serializationCls = macro class {
			public function unserialize(bi:haxe.io.BytesInput)
				$b{inExprlist};
